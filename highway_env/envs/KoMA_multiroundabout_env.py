...
class KoMAMultiRoundAboutEnv(AbstractEnv):
    ...
    def _is_terminated(self) -> bool:
        ...
        for i in range(self.config["controlled_vehicles_count"]):
            is_terminate = self.controlled_vehicles[i].crashed
            if  self.controlled_vehicles[i].lane_index == ('ex', 'exs', 0):
                done =True
            is_terminated = (is_terminate | is_terminated | done)
    ...
    def _make_vehicles(self) -> None:
        ...
        ego_vehicle = self.action_type.vehicle_class(self.road,
                                                     ego_lane.position(80 + np.random.rand()* 10 - 5, 0),
                                                     speed=10 + np.random.rand()* 5 - 2.5,
                                                     heading=ego_lane.heading_at(140))
        try:
            ego_vehicle.plan_route_to("exs")
        ...
        # Ego3-0
        ego3_lane = self.road.network.get_lane(("ser", "ses", 0))
        ego3_vehicle = other_vehicles_type(self.road,
                                           ego3_lane.position(60 + np.random.rand()* 10 - 5, 0),
                                           speed=10 + np.random.rand()* 5 - 2.5,
                                           heading=ego3_lane.heading_at(140))
        try:
            ego3_vehicle.plan_route_to("exs")
        except AttributeError:
            pass
        self.road.vehicles.append(ego3_vehicle)


        #Ego2-0
        ego2_0_lane = self.road.network.get_lane(("wer", "wes", 0))
        ego2_0_vehicle = other_vehicles_type(self.road,
                                            ego2_0_lane.position(130 + np.random.rand()* 10 - 5, 0),
                                            speed=10 + np.random.rand()* 5 - 2.5,
                                            heading=ego2_0_lane.heading_at(140))
        try:
            ego2_0_vehicle.plan_route_to("exs")
        except AttributeError:
            pass
        self.road.vehicles.append(ego2_0_vehicle)


        #Ego2-1
        ego2_1_lane = self.road.network.get_lane(("wer", "wes", 0))
        ego2_1_vehicle = other_vehicles_type(self.road,
                                            ego2_1_lane.position(90 + np.random.rand()* 10 - 5, 0),
                                            speed=10 + np.random.rand()* 5 - 2.5,
                                            heading=ego2_1_lane.heading_at(140))
        try:
            ego2_1_vehicle.plan_route_to("exs")
        except AttributeError:
            pass
        self.road.vehicles.append(ego2_1_vehicle)


        # Ego2-2
        ego2_2_lane = self.road.network.get_lane(("wer", "wes", 0))
        ego2_2_vehicle = other_vehicles_type(self.road,
                                            ego2_2_lane.position(70 + np.random.rand()* 10 - 5, 0),
                                            speed=10 + np.random.rand()* 5 - 2.5,
                                            heading=ego2_2_lane.heading_at(140))
        try:
            ego2_2_vehicle.plan_route_to("exs")
        except AttributeError:
            pass
        self.road.vehicles.append(ego2_2_vehicle)


        # Ego2-3
        ego2_3_lane = self.road.network.get_lane(("wer", "wes", 0))
        ego2_3_vehicle = other_vehicles_type(self.road,
                                            ego2_3_lane.position(110 + np.random.rand()* 10 - 5, 0),
                                            speed=8,
                                            heading=ego2_3_lane.heading_at(140))
        try:
            ego2_3_vehicle.plan_route_to("exs")
        except AttributeError:
            pass
        self.road.vehicles.append(ego2_3_vehicle)
