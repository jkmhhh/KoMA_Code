...
class KoMAMergeGeneralizationEnv(AbstractEnv):
    ...
    def _is_terminated(self) -> bool:
        ...
        for i in range(self.config["controlled_vehicles_count"]):
            is_terminate = (self.controlled_vehicles[i].crashed | bool(self.controlled_vehicles[1].position[0] > 370)) #匝道车辆汇入成功
            is_terminated = (is_terminate | is_terminated)
        return is_terminated

    def _collision_idlist(self) -> int:
        collision_list = []
        for i in range(self.config["controlled_vehicles_count"]):
            collision_list.append(self.controlled_vehicles[i].crashed)
    ...
    def _make_vehicles(self) -> None:
        ...
        spawn_points_s = [[170, 0], [170, 1], [200, 0], [200, 1], [230, 0], [230, 1], [260, 0], [260, 1]] #150 230 350 500
        ...
        ego_vehicle = self.action_type.vehicle_class(road,
                                                     road.network.get_lane(("b", "c", 2)).position(1, 0),
