...
class EnvScenario:
    def __init__(
        ...
        # The following four variables are used to determine whether a vehicle is within ego's hazardous visual range
    ...
    def processNormalLane(self, lidx: LaneIndex) -> str:
        ...
        if lidx == ('k', 'b', 0):
            background = "You are driving on the entrance ramp of the highway, approaching the section where you can merge onto the highway. Your goal is to safely merge onto the highway before reaching the end of the ramp, so please make sure there is no collision with other vehicles before deciding to change lanes.\n"
        elif lidx == ('b', 'c', numLanes - 1):
            background = f"You are driving on the entrance ramp of the highway and have reached the section where you can merge onto the highway.The end of the ramp is {116.5 - self.getLanePosition(self.ego):.2f} meters ahead of you, you must turn left to merge onto the highway before reaching the end of ramp, otherwise you will have a collision. Your goal is to merge onto the highway safely in time.  Please make sure there is no collision with other vehicles before changing lanes.\n"
        else:
            background = "You are driving on a highway. Please note that vehicles will be merging from on-ramps into the far right lane, which may pose a safety risk to you. You need to avoid colliding with merging vehicles.\n"
    ...
    def getSVRelativeState(self, sv: IDMVehicle) -> str:
        # CAUTION: in pygame, the y-axis is inverted, with the positive direction of the y-axis being downward
    ...
    def processSingleLaneSVs(self, SingleLaneSVs: List[IDMVehicle]):
        # Returns the nearest vehicle in front and the nearest vehicle behind in the current lane, or None if not available
        if SingleLaneSVs:
            aheadSVs = []
            behindSVs = []
    ...
    def processSVsNormalLane(
        ...
        # Keep only the few vehicles that are closest to the ego vehicle.
    ...
    def describeSVNormalLane(self, currentLaneIndex: LaneIndex) -> str:
        # When the ego vehicle is on a StraightLane, lane information is crucial and needs to be processed:
        # First, determine whether the vehicle is on the same road as the ego vehicle.
        # If it is on the same road, then determine which lane it is on.
        # If it is not on the same road, then determine whether it is on the next_lane.
        # If it is not on the next_lane, then disregard the information of this vehicle directly.
        # If it is on the next_lane, then tally the relative motion status of this vehicle concerning the ego vehicle.
        sideLanes = self.network.all_side_lanes(currentLaneIndex)
        nextLane = self.network.next_lane(
            currentLaneIndex, self.ego.route, self.ego.position
        )
        ...
        if not surroundVehicles:
            ...
            for sv in surroundVehicles:
                ...
                if lidx in sideLanes:
                    if lidx == ('b', 'c', 2):
                        SVDescription += f"- Vehicle `{id(sv) % 1000}` will turn left when he find a safe distance in a few seconds. "
                    if lidx == currentLaneIndex:
                        ...
                        laneRelative = lidx[2] - currentLaneIndex[2]
                        if laneRelative == 1:
                            # laneRelative = 1 indicates that the vehicle is traveling in the right lane of ego
                            if sv in validVehicles:
                                SVDescription += f"- Vehicle `{id(sv) % 1000}` is driving on the lane to your right and {self.getSVRelativeState(sv)}. "
                            else:
                                continue
                        elif laneRelative == -1:
                            # laneRelative = -1 indicates that the vehicle is traveling in the left lane of ego
                            if sv in validVehicles:
                    ...
                    continue
                SVDescription += f"The centre position of it is `({sv.position[0]:.2f}, {sv.position[1]:.2f})`, length is {sv.LENGTH} meters, speed is {sv.speed:.2f} m/s, acceleration is {sv.action['acceleration']:.2f} m/s^2, and lane position is {self.getLanePosition(sv):.2f} m.\n"
            if SVDescription:
